...
const Equipe = () => {
  ...
  const handleAddMember = async () => {
    ...
    try {
      ...
      if (ownerUserId) {
        const newTotalMembers = team.length + 1;
        const billingResult = await updateBillingSeats(ownerUserId, newTotalMembers);

        if (billingResult.success) {
          if (billingResult.isUpgrade) {
            toast.info(`Facturation mise à jour: ${billingResult.previousQuantity} → ${billingResult.newQuantity} sièges`);
          }
          // Refresh subscription data
          await refreshSubscription();
        } else {
          console.warn("[Billing] Failed to update seats:", billingResult.error);
          // Don't block member creation, just log the error
        }
      }
