-- Fix persistent COALESCE date/text mismatch by casting planned_date to text explicitly
...
BEGIN
  ...
  THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM public.jobs WHERE quote_id = NEW.id) THEN
      ...
      -- Get the job date, handling NULL properly (cast planned_date to text to avoid type mismatch)
      v_job_date := COALESCE(NEW.planned_date::text, to_char(CURRENT_DATE, 'YYYY-MM-DD'));
      ...
      IF v_job_id IS NOT NULL AND NEW.lignes IS NOT NULL AND jsonb_typeof(NEW.lignes) = 'array' THEN
        FOR v_line IN SELECT * FROM jsonb_array_elements(NEW.lignes) LOOP
          ...
          -- Copy if type matches or inventory link exists
