-- Fix COALESCE type mismatch error in create_job_on_quote_accept trigger
...
BEGIN
  ...
  THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM public.jobs WHERE quote_id = NEW.id) THEN
      ...
      -- Get the job date, handling NULL properly
      v_job_date := COALESCE(NEW.planned_date, to_char(CURRENT_DATE, 'YYYY-MM-DD'));
