/**
 * Quote HTML Renderer - Backend Version (Deno/Edge Functions)
 *
 * IMPORTANT: Ce fichier est une COPIE de src/lib/quoteHtmlRenderer.ts
 * adapté pour fonctionner dans les Edge Functions Supabase (Deno).
 *
 * Si vous modifiez le rendu, vous DEVEZ mettre à jour les DEUX fichiers :
 * - src/lib/quoteHtmlRenderer.ts (frontend)
 * - supabase/functions/_shared/quoteHtmlRenderer.ts (backend - ce fichier)
 *
 * Le but est de garantir que le rendu soit IDENTIQUE partout.
 */
...
// ============================================================================
// CONVERSION FUNCTION FOR EXISTING QUOTE DATA
// ============================================================================
...
export function convertQuoteToRenderData(quote: any): QuoteRenderData {
  ...
  return {
    ...
    acompte: quote.required_deposit_ht || quote.acompte || 0,

    lignes: lines,

    message_client: quote.client_message || quote.message_client || "",
    conditions: quote.conditions || quote.notes_legal || "",

    signature: quote.quote_signatures?.[0] ? {
      signed_at: quote.quote_signatures[0].signed_at,
      signer_name: quote.quote_signatures[0].signer_name,
      signature_image_url: quote.quote_signatures[0].signature_image_url,
    } : undefined,
  };
}
 ...
 */
export function getDefaultTemplate(): Partial<DocumentTemplate> {
  return {
    type: "QUOTE",
